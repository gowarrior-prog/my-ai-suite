...
export default function Sidebar({ isOpen, setIsOpen, onChatSelect, currentChatId, onCreateNewChat }: SidebarProps) {
  ...
  // Load Current User (sidebar footer mein naam/avatar dikhane ke liye)
  useEffect(() => {
  ...
  return (
    <>
      ...
      <div className={`fixed lg:relative inset-y-0 left-0 z-50 ${isOpen ? 'w-64 translate-x-0' : 'w-20 -translate-x-full lg:translate-x-0 lg:w-20'} bg-[#050709] h-screen border-r border-white/[0.03] flex flex-col transition-all duration-300 ease-in-out select-none`}>
        ...
        <div className="p-3 space-y-1">
          ...
          <Link href="/" className={`flex items-center gap-3 w-full px-3 py-2.5 rounded-xl transition-all text-xs ${pathname === '/' ? 'bg-[#151b26] text-cyan-400 border border-cyan-500/10 font-bold' : 'text-gray-500 hover:bg-white/[0.02] hover:text-gray-300'}`}>
            <Bot size={16} />
        ...
        {pathname === '/' && (
          <div className="px-3 py-1">
            <button 
              ...
              className="flex items-center gap-3 w-full px-3 py-2 rounded-xl bg-white/[0.03] border border-white/5 hover:bg-white/[0.06] text-gray-300 transition-all font-semibold text-xs justify-center md:justify-start"
            >
              <Plus size={16} />
        ...
        {isOpen && pathname === '/' && (
          <div className="flex-1 overflow-y-auto px-3 space-y-0.5 py-3 custom-scrollbar border-t border-white/[0.02] mt-2">
            ...
            {chats.length === 0 && <p className="text-gray-600 text-[11px] px-3 py-2 italic">No chats yet</p>}
            
            {chats.map(chat => (
              <div 
                key={chat.id} 
                onClick={() => onChatSelect(chat)} 
                className={`px-3 py-2 rounded-xl hover:bg-white/[0.02] cursor-pointer flex justify-between items-center group transition-all ${currentChatId === chat.id ? 'bg-white/[0.04] text-white font-medium' : 'text-gray-500 hover:text-gray-300'}`}
              >
                ...
                <button 
                  ...
                  className="opacity-0 group-hover:opacity-100 p-0.5 hover:bg-white/10 rounded-md text-gray-600 hover:text-red-400 transition-all"
                >
                  <Trash2 size={12} />
                </button>
              </div>
        ...
        <div className="p-3 border-t border-white/[0.03] bg-[#050709] mt-auto">
          <div 
            ...
            className="flex items-center justify-between gap-3 p-2 rounded-xl hover:bg-white/[0.02] transition-colors cursor-pointer group"
          >
